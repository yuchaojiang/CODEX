normalizeParallel <- function( Y_qc, gc_qc, K ) 
{
    if (max(K) > ncol(Y_qc)) 
...
normalizeSingleK <- function(Y_qc, gc_qc, k) {
  ...
  betahatmat <- matrix(nrow = nrow(Y_qc), ncol = ncol(Y_qc), 
                       data = betahat, byrow = FALSE)
  Yhat <- round(fhat * Nmat * betahatmat * exp(ghat %*% t(hhat)),0)
  AIC <- 2 * sum(Y_qc * log(pmax(Yhat,1)) - Yhat) - 
    2 * (length(ghat) + length(hhat))
  BIC <- 2 * sum(Y_qc * log(pmax(Yhat,1)) - Yhat) - (length(ghat)
                                                     + length(hhat)) * log(length(Y_qc))
  RSS <- sum((Y_qc - Yhat)^2/length(Y_qc))
  message("AIC", k, " = ", round(AIC, 3))
  message("BIC", k, " = ", round(BIC, 3))
  message("RSS", k, " = ", round(RSS, 3), "\n")
